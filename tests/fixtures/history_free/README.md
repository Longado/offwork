...
`runs/2026-08-29/` preserves the first completed three-case exercise after the prompt became self-contained. Each Agent received only this prompt, one explicit temporary project path, and the exact recorded CLI envelope. The records preserve the exact responses and measured CLI command data. Adjacent evidence files preserve matching before/after HEAD, Git-index digest, project-file digest, and acceptance event state instead of relying only on a summary claim. These Agent decisions are recovery evidence, not PM acceptance.
